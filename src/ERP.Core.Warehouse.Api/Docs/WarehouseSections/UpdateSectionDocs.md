...
| `section_id`   | `guid`   | Sí        | Identificador de la sección a actualizar. Debe existir en ese almacén y no estar eliminada. |
...
| Capacidad de almacén | El almacén debe tener `warehouse_capacity`; se actualiza tras el cálculo. Recibe 400: `El almacén no tiene capacidad registrada`. |
| Rol `Supervisor` | Recibe 400: `No tienes permiso para realizar esta acción`. |
| Sección inválida | Recibe 400: `La sección indicada no existe o no pertenece al almacén.` (también si está eliminada). |
