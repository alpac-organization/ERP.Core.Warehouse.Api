...
| **Descripción** | Actualiza los campos enviados de la sección. Si cambian `width`, `length` o `section_type`, recalcula la capacidad de la sección y actualiza `warehouse_capacity`. |
...
| `section_id`   | `guid`   | Sí        | Identificador de la sección a actualizar. Debe existir en ese almacén. |
...
| Parámetro      | Tipo                 | Requerido | Descripción |
|----------------|----------------------|-----------|-------------|
| `code`         | `string`             | No        | Nuevo código. Si se envía, no puede estar vacío, máximo 50 caracteres y debe ser único en el almacén. |
| `section_type` | `enum (SectionType)` | No        | Nuevo tipo de sección. Debe ser un valor válido del enum. Si se envía, dispara recálculo de capacidad. |
...
| Recálculo | Ocurre si viene `width`, `length` o `section_type`. Usa el valor enviado o el `width`/`length` de `section_capacity` y el `section_type` actual. |
| `SectionType` | Enum de `ERP.Core.Database.Domain.Enums`. Este command no tiene `JsonNumberEnumConverter`; aplica el converter global. FluentValidation: `El tipo de sección no es válido.` El catálogo completo no está definido en este repositorio. |
| Capacidad inexistente | Si hay que recalcular y la sección no tiene `section_capacity`, se crea un registro nuevo. |
...
| Sección inválida | Recibe 400: `La sección indicada no existe o no pertenece al almacén.` |
