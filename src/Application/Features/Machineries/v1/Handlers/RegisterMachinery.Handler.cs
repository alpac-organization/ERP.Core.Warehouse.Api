...
public class RegisterMachineryHandler(IUnitOfWork unitOfWork, IErrorManager errorManager, IMapper mapper, ILogger<RegisterMachineryHandler> logger) : BaseValidatorHandler<MachineryCommand, bool>(unitOfWork, errorManager)
{
    public override async Task<bool> Handle(MachineryCommand request, CancellationToken cancellationToken)
    {
        logger.LogInformation("🚀Iniciando proceso de registro de maquinaria.");
        ...
        var machinery = mapper.Map<Machinery>(request);
