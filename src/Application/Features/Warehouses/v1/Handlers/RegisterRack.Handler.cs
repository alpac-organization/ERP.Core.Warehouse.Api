    ...
    : BaseValidatorHandler<RacksBulkCommand, bool>(unitOfWork, errorManager)
{
    public override async Task<bool> Handle(RacksBulkCommand request, CancellationToken cancellationToken)
    {
        ...
        if (sectionInfo.StorageType == SectionStorageType.Lots)
            return _errorManager.ThrowBadRequest<bool>("Esta sección está configurada para Tramos (Lots). Crea una sección independiente para los Racks.", "ERP:SECTION_STORAGE_MISMATCH");
        ...
        if (duplicatedCodesInRequest.Count > 0)
            return _errorManager.ThrowBadRequest<bool>($"Se enviaron códigos duplicados en la solicitud: {string.Join(", ", duplicatedCodesInRequest)}.", "ERP:RACK_CODE_DUPLICATED_IN_REQUEST");
