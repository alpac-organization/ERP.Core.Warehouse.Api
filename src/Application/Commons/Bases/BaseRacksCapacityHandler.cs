...
public abstract class BaseRacksCapacityHandler<TRequest>(
    ...
    where TRequest : IRequest<bool>
{
    ...
    protected async Task<(Racks? Rack, bool IsValid, bool ErrorResponse)> GetExistingRackAsync(
        Guid rackId,
        Guid sectionId,
        CancellationToken cancellationToken)
    {
        ...
        if (rack is null)
            return (null, false, _errorManager.ThrowBadRequest<bool>(
                "El rack no fue encontrado.", "ERP:RACK_NOT_FOUND"));

        return (rack, true, default);
    }
